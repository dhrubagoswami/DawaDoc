    ...
    <div className="min-h-screen bg-linear-to-b from-cream-100 via-cream-50 to-sage-50">
      <header className="flex items-center justify-between px-5 py-5 sm:px-8">
        <div className="flex items-center gap-2.5">
          <Logo />
          <span className="text-lg font-semibold tracking-tight text-ink-900">DawaDoc</span>
        </div>
      ...
      <section className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-5 pb-20 pt-10 text-center sm:px-8 sm:pt-16">
        ...
        <h1 className="max-w-2xl text-4xl font-bold leading-[1.15] tracking-tight text-ink-900 sm:text-5xl">
          {t.heroTitle} <span className="text-sage-600">{t.heroTitleAccent}</span>
        </h1>
        <p className="max-w-xl text-lg leading-relaxed text-ink-700/80">{t.heroSubtitle}</p>
        <div className="mt-2 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-ink-700/60">{t.heroReassurance}</p>
        </div>
      </section>

      {/* How it works */}
      <section className="mx-auto max-w-5xl px-5 pb-20 sm:px-8">
        <h2 className="mb-10 text-center text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">
          {t.howItWorksTitle}
        </h2>
        <div className="grid gap-6 sm:grid-cols-3">
          {t.steps.map((step, i) => (
            <div
              key={step.title}
              className="relative flex flex-col gap-3 rounded-3xl bg-white p-7 shadow-sm ring-1 ring-black/5"
            >
              ...
              <p className="text-sm leading-relaxed text-ink-700/75">{step.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-5 pb-20 sm:px-8">
        <div className="mb-10 text-center">
          <h2 className="text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">{t.featuresTitle}</h2>
          <p className="mt-2 text-base text-ink-700/70">{t.featuresSubtitle}</p>
        </div>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {t.features.map((feature) => (
            <div
              key={feature.title}
              className="flex flex-col gap-2 rounded-2xl bg-white/70 p-6 ring-1 ring-sage-100"
            >
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-clay-100 text-clay-600">
                <FeatureDot />
              </div>
              <h3 className="text-base font-semibold text-ink-900">{feature.title}</h3>
              <p className="text-sm leading-relaxed text-ink-700/75">{feature.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-2xl px-5 pb-20 text-center sm:px-8">
        <div className="rounded-3xl bg-sage-50 p-8 ring-1 ring-sage-100">
          <h2 className="text-xl font-semibold text-ink-900">{t.trustTitle}</h2>
          <p className="mt-3 text-sm leading-relaxed text-ink-700/80">{t.trustBody}</p>
        </div>
      </section>

      {/* Final CTA */}
      <section className="mx-auto flex max-w-2xl flex-col items-center gap-5 px-5 pb-24 text-center sm:px-8">
        <h2 className="text-3xl font-bold tracking-tight text-ink-900 sm:text-4xl">{t.finalTitle}</h2>
        <p className="text-lg text-ink-700/80">{t.finalSubtitle}</p>
        <button
          type="button"
          onClick={goToScan}
      ...
      <footer className="px-5 pb-8 text-center text-xs text-ink-700/50">DawaDoc</footer>
    </div>
  );
}
